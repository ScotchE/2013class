...
?>
	...
	@foreach ($scratches as $key => $scratch)
		...
		@if ( $scratch->pilote->naissance < '1978-01-01')

		<?php

			$points = Point::find($position)['points'];
			if (isset($classement[$scratch->pilote->nom])) {
				$classement[$scratch->pilote->nom] += $points;
			} else {
				$classement[$scratch->pilote->nom] = $points;
			}
			$classements[$course->id][$scratch->pilote->nom] = Point::find($position)['points'];
		?>

		<tr>
			<td>{{{ $position }}} / {{ Point::find($position++)['points'] }} </td>
